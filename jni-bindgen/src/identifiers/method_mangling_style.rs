...
fn method_mangling_style_mangle_test() {
    for &(name, sig, java, java_short, java_long, rust, rust_short, rust_long) in &[
        ...
        assert_eq!(
            MethodManglingStyle::Rustify.mangle(name, sig).unwrap(),
            rust
        );
        assert_eq!(
...
impl MethodManglingStyle {
    pub fn mangle(
    ...
    ) -> Result<String, IdentifierManglingError> {
        let name = match name {
            "" => {
        ...
        match self {
            ...
            MethodManglingStyle::JavaShortSignature => Ok(javaify_identifier(
                &format!("{}{}", name, short_sig(descriptor))[..],
            )?),
            MethodManglingStyle::JavaLongSignature => Ok(javaify_identifier(
                &format!("{}{}", name, long_sig(descriptor))[..],
            )?),

            MethodManglingStyle::Rustify => Ok(rustify_identifier(name)?),
            MethodManglingStyle::RustifyShortSignature => Ok(rustify_identifier(
                &format!("{}{}", name, short_sig(descriptor))[..],
            )?),
            MethodManglingStyle::RustifyLongSignature => Ok(rustify_identifier(
                &format!("{}{}", name, long_sig(descriptor))[..],
            )?),
        }
    }
