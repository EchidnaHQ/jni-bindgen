...
impl<'a> RustIdentifier<'a> {
    ...
    pub fn from_str(s: &'a str) -> RustIdentifier<'a> {
        match s {
            ...
            s => {
                if String::from(s.chars().next().unwrap())
                    .parse::<usize>()
                    .is_ok()
                {
...
fn rust_identifier_from_str() {
    ...
    assert_eq!(
        ...
        RustIdentifier::NonIdentifier("föo")
    );
    assert_eq!(
        RustIdentifier::from_str(""),
        RustIdentifier::NonIdentifier("")
    );
    assert_eq!(
        RustIdentifier::from_str("_"),
        RustIdentifier::NonIdentifier("_")
    );
    assert_eq!(
        RustIdentifier::from_str("_f"),
        RustIdentifier::Identifier("_f")
    );
    assert_eq!(
        RustIdentifier::from_str("_1"),
        RustIdentifier::Identifier("_1")
    );
    assert_eq!(
        RustIdentifier::from_str("1_"),
        RustIdentifier::NonIdentifier("1_")
    );
}
...
fn is_rust_identifier(s: &str) -> bool {
    ...
    // First char
    let first_char = if let Some(ch) = chars.next() {
        ch
    } else {
        return false;
    };
