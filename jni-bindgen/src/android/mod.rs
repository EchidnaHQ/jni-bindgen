...
impl From<RawApiLevelRange> for ApiLevelRange {
    fn from(value: RawApiLevelRange) -> Self {
        Self(value)
    }
}
impl Deref for ApiLevelRange {
    fn deref(&self) -> &Self::Target {
        &self.0
    }
    type Target = RawApiLevelRange;
}
impl DerefMut for ApiLevelRange {
...
impl FromStr for ApiLevelRange {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if let Ok(single) = s.parse::<u32>() {
            ...
            let (start, end) = (
                start.parse::<u32>().map_err(|_| err)?,
                end[1..].parse::<u32>().map_err(|_| err)?,
            );
            Ok(Self(start..=end))
...
fn parse() {
    assert_eq!(
        ApiLevelRange(8..=27),
        "8-27".parse::<ApiLevelRange>().unwrap()
    );
    assert_eq!(
        ApiLevelRange(8..=27),
        "8..=27".parse::<ApiLevelRange>().unwrap()
    ...
    assert_eq!(
        ApiLevelRange(8..=8),
        "8-8".parse::<ApiLevelRange>().unwrap()
    );
    assert_eq!(
        ApiLevelRange(8..=8),
        "8..=8".parse::<ApiLevelRange>().unwrap()
