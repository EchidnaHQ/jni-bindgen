...
impl<'a> Method<'a> {
    pub fn new(
        context: &Context,
        class: &'a jreflection::Class,
        java: &'a jreflection::Method,
    ) -> Self {
        let mut result = Self {
            class,
            java,
    ...
    pub fn emit(
        ...
        let ignored = context
            ...
            .contains(&java_class_method)
            || context
                .config
                .ignore_class_method_sigs
                .contains(&java_class_method_sig);
        ...
        let mut params_decl = if self.java.is_constructor() || self.java.is_static() {
            match context.config.codegen.static_env {
                config::toml::StaticEnvStyle::Explicit => {
                    String::from("__jni_env: &'env __jni_bindgen::Env")
                }
                config::toml::StaticEnvStyle::__NonExhaustive => {
                    emit_reject_reasons
                        .push("ERROR:  StaticEnvStyle::__NonExhaustive is invalid, silly goose!");
                    String::new()
                }
            }
        } else {
                ...
                method::Type::Array { levels, inner } => {
                    ...
                    match inner {
                        method::BasicType::Boolean => {
                            buffer.push_str("__jni_bindgen::BooleanArray")
                        }
        ...
        let mut ret_decl = match descriptor.return_type() {
            ...
            method::Type::Single(method::BasicType::Class(class)) => {
                ...
                match context.java_to_rust_path(class) {
                    ...
                    Err(_) => {
                        emit_reject_reasons.push(
                            "ERROR:  Failed to resolve JNI path to Rust path for return type",
                        );
                        format!("{:?}", class)
                    }
                }
            ...
            method::Type::Array { levels, inner } => {
                let mut buffer =
                    "__jni_bindgen::std::option::Option<__jni_bindgen::Local<'env, ".to_owned();
                for _ in 0..(levels - 1) {
                    buffer.push_str("__jni_bindgen::ObjectArray<");
                }
                match inner {
        ...
        if self.java.is_constructor() {
            if descriptor.return_type() == method::Type::Single(method::BasicType::Void) {
                ...
                ret_decl = match context.java_to_rust_path(self.class.path.as_id()) {
                    ...
                    Err(_) => {
                        emit_reject_reasons
                            .push("ERROR:  Failed to resolve JNI path to Rust path for this type");
        ...
        if self.java.is_constructor() || self.java.is_static() {
            match context.config.codegen.static_env {
                config::toml::StaticEnvStyle::Explicit => {}
                config::toml::StaticEnvStyle::__NonExhaustive => {
                    writeln!(out, "{}    let __jni_env = ...?;", indent)?
                } // XXX
            };
        } else {
            writeln!(
