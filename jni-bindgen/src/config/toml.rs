...
impl File {
    ...
    pub fn read_str(buffer: &str) -> io::Result<Self> {
        let file: File =
            toml::from_str(buffer).map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
        Ok(file)
    }
...
fn load_well_configured_toml() {
    ...
    assert_eq!(
        file.documentation.patterns[0].class_namespace_separator,
        "/"
    );
    assert_eq!(
        file.documentation.patterns[0].class_inner_class_seperator,
        "."
    );
    assert_eq!(
        file.documentation.patterns[0].argument_namespace_separator,
        "."
    );
    assert_eq!(
        file.documentation.patterns[0].argument_inner_class_seperator,
        "."
    );
    ...
    assert_eq!(
        file.documentation.patterns[1].class_namespace_separator,
        "/"
    );
    assert_eq!(
        file.documentation.patterns[1].class_inner_class_seperator,
        "."
    );
    assert_eq!(
        file.documentation.patterns[1].argument_namespace_separator,
        "."
    );
    assert_eq!(
        file.documentation.patterns[1].argument_inner_class_seperator,
        "."
    );
...
fn load_minimal_toml() {
    ...
    assert_eq!(file.codegen.static_env, StaticEnvStyle::Explicit);
    assert_eq!(
        file.codegen.method_naming_style,
        MethodManglingStyle::Rustify
    );
    assert_eq!(
        file.codegen.method_naming_style_collision,
