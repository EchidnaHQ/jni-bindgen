...
macro_rules! primitive_array {
    (#[repr(transparent)] pub struct $name:ident = $type_str:expr, $type:ident { $new_array:ident $set_region:ident $get_region:ident } ) => {
        ...
        impl PrimitiveArray<$type> for $name {
            ...
            fn len(&self) -> usize {
                unsafe {
                    (**self.0.env).GetArrayLength.unwrap()(self.0.env as *mut _, self.0.object)
                        as usize
                }
            }
...
///
#[repr(transparent)]
pub struct ObjectArray<T: AsValidJObjectAndEnv, E: ThrowableType>(
    ObjectAndEnv,
    PhantomData<(T, E)>,
);
...
impl<T: AsValidJObjectAndEnv, E: ThrowableType> ObjectArray<T, E> {
    ...
    pub fn len(&self) -> usize {
        unsafe {
            (**self.0.env).GetArrayLength.unwrap()(self.0.env as *mut _, self.0.object) as usize
        }
    }
